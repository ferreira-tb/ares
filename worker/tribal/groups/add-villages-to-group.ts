...
async function addVillagesToGroup(port: MessagePort, data: AddVillageToGroupData) {
    try {
        const table = document.queryStrict<HTMLTableElement>(tableSelector);
        ...
        rawRows.forEach((row) => {
            const villageAnchor = row.queryStrict<HTMLAnchorElement>('a[href*="village=" i]');
        ...
        for await (const row of expandRows(rows)) {
            ...
            const groupInput = row.queryStrict<HTMLInputElement>(inputSelector);
            if (groupInput.checked) continue;
            groupInput.click();

            const submitSelector = 'input[type="submit"].btn';
            const submit = row.queryStrict<HTMLInputElement>(submitSelector);
            submit.click();
        }
...
async function* expandRows(rows: Map<number, HTMLTableRowElement>) {
    ...
    for (const [villageId, row] of rows.entries()) {
        ...
        const groupEditRow = table.queryStrict<HTMLTableRowElement>(rowSelector);
        const editAnchor = row.queryStrict<HTMLAnchorElement>(anchorSelector);
